...
public class SearchModel {
    ...
    private ArrayList<AlphaBetaNode> generateMovesForAll(boolean isMax) {
        ...
        for(int i=0;i<32;i++) {
            ...
            if (!isMax && piece.color == 'b') continue;
            if(!piece.flag)
            for (int[] nxt : Rules.getNextMove(piece.first, piece.position, board))
                moves.add(new AlphaBetaNode(piece.first, piece.position, nxt));
            else for (int[] nxt : Rules.getNextMove(piece.second, piece.position, board))
                moves.add(new AlphaBetaNode(piece.second, piece.position, nxt));
        }
        return moves;
    }
